...
function testDrive() {
  ...
  const folder = getOrCreateFolder('Workspace_Automation_Logs');
  listFilesByFolder('Workspace_Automation_Logs');
}
...
function testSheets() {
  ...
  createSpreadsheetWithData('Workspace 자동화 리포트', headers, rows);
}
...
function testDocs() {
  ...
  createDocumentReport('Workspace 자동화 일일보고서', sections);
}
